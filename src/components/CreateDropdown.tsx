...
import { Fragment, useEffect } from "react";
 ...
 export default function CreateDropdown() {
  ...
  <Menu as="div" className="relative inline-block md:w-72 lg:w-96 text-left">
      ...
      >
        <Menu.Items className="absolute hover:cursor-pointer left-0 z-10 mt-2 lg:w-96 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            <Menu.Item>
              {({ active }) => (
                <a
            ...
            <Menu.Item>
              {({ active }) => (
                <a
                onClick={()=> setServiceLine("Orthopedics")}
                className={classNames(
                    active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                    'block px-4 py-2 text-sm'
                  )}
